...
// next node to the current node. And a Weak (weak Reference Counted)
// pointer to reference the previous node without owning it.
...
impl<T> List<T> {
    ...
    fn append(&mut self, data: T) {
        match self.first.as_mut() {
            Some(first) => {
                if let Some(last) = Node::append(first, data) {
                    self.last = Some(last);
                }
            }
            None => {
